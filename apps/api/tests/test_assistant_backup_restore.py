...
def test_empty_media_snapshot_and_fresh_restore(tmp_path: Path) -> None:
    settings = _prepared_settings(tmp_path)
    with sqlite3.connect(tmp_path / "live" / "content.db") as connection:
        connection.execute("DELETE FROM media_assets")
    media = Path(settings.media_root)
    (media / ("a" * 32) / "image.webp").unlink()
    (media / ("a" * 32)).rmdir()
    snapshot = tmp_path / "empty-snapshot"
    manifest = create_consistent_snapshot(settings=settings, snapshot_dir=snapshot)
    assert manifest["media"]["file_count"] == 0
    assert (snapshot / "media").is_dir()
    assert verify_snapshot(snapshot) == manifest
    report = restore_to_fresh_target(
        settings=settings, snapshot_dir=snapshot,
        destination_database=tmp_path / "restored" / "content.db",
        destination_media=tmp_path / "restored" / "media",
        destination_runtime=tmp_path / "restored" / "runtime.db",
    )
    assert report["runtime_created_empty"] is True
    assert report["media_file_count"] == 0
