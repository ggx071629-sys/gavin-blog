...
@pytest.mark.parametrize("close_notification", [True, False])
def test_sse_drains_committed_answer_when_live_notification_is_missing(
    tmp_path: Path, monkeypatch, close_notification: bool,
) -> None:
    from app.assistant.hub import TurnHub

    publish = TurnHub.publish

    def drop_answer(self, turn_id, event_name, data_json):
        if event_name != "answer":
            publish(self, turn_id, event_name, data_json)

    monkeypatch.setattr(TurnHub, "publish", drop_answer)
    if not close_notification:
        monkeypatch.setattr(TurnHub, "close", lambda self, turn_id: None)
    with _online_client(tmp_path) as client:
        _publish_and_index(client)
        csrf = _create_session(client)
        status, body = _sse_body(
            client, csrf, "What do you write about?", idem="missing-live-answer",
        )
        assert status == 200
        assert body.count("event: answer\n") == 1
        assert '"code":"answered"' in body
